...
const checkersMustKill = (evt, allChecker, currentChecker) => {
    ...
    for (let i = 0; i < checkerByColor.length; i++) {
        ...
        if (nextCellToDownRight && nextCellToDownRight?.color !== selectedColor && !throughOneCellToDownRight) {
        ...
        if (nextCellToDownLeft && nextCellToDownLeft?.color !== selectedColor && !throughOneCellToDownLeft) {
            if (!checkerWhoMustKill.includes(checkerByColor[i].id)) {
                checkerWhoMustKill.push(checkerByColor[i].id)
            }
        }
        ...
        if (nextCellToUpRight && nextCellToUpRight?.color !== selectedColor && !throughOneCellToUpRight) {
            if (!checkerWhoMustKill.includes(checkerByColor[i].id)) {
                checkerWhoMustKill.push(checkerByColor[i].id)
            }
        }
        ...
        if (nextCellToUpLeft && nextCellToUpLeft?.color !== selectedColor && !throughOneCellToUpLeft) {
            if (!checkerWhoMustKill.includes(checkerByColor[i].id)) {
                checkerWhoMustKill.push(checkerByColor[i].id)
            }
        }
